<div class="row">
    <div class="mb-5 col-lg-12">
        ...
        {{ Form::select('lang_id', getLanguage(), isset($gallery) ? $gallery->lang_id :null, ['class' => 'form-select', 'id' => 'langId', 'placeholder' => __('messages.common.select_language'), 'data-control' => 'select2','required']) }}
    </div>
    <div class="mb-5 col-lg-12">
        {{ Form::label('album',__('messages.gallery.album').' :', ['class' => 'form-label required mb-3']) }}
        {{ Form::select('album_id',[],null,['class' => 'form-select', 'id' => 'albumId', 'placeholder' => __('messages.gallery.select_album'), 'data-control' => 'select2','required']) }}
    </div>
    <div class="mb-5 col-lg-12">
        {{ Form::label('category',__('messages.gallery.category').' :', ['class' => 'form-label required mb-3']) }}
        {{ Form::select('category_id', [],null,['class' => 'form-select', 'id' => 'categoryId', 'placeholder' =>__('messages.common.select_category'), 'data-control' => 'select2','required']) }}
    </div>
    <div class="mb-5 col-lg-12">
        {{ Form::label('title',__('messages.gallery.title').' :', ['class' => 'form-label required mb-3']) }}
        {{ Form::text('title',isset($gallery) ? $gallery->title :null, ['class' => 'form-control', 'id' => 'titleId', 'placeholder' =>__('messages.gallery.title'),'required']) }}
    </div>
    <div class="mb-5 col-lg-12">
        {{ Form::label('image',__('messages.gallery.image').' :', ['class' => 'form-label required mb-3'])}}
        <input type="file" class="form-control" id="newImage" name="images[]" multiple="multiple"

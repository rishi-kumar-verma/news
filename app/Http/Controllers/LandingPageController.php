...
class LandingPageController extends AppBaseController
{
    ...
    public function index()
    {
        ...
        $data['latestPosts'] = Post::with('user', 'category')->whereVisibility(Post::VISIBILITY_ACTIVE)->orderBy('created_at')->take(4)->get();
    ...
    public function detailPage(Request $request,$slug, $id = null)
    {
        ...
        $post = Post::with(['category', 'postArticle', 'postGalleries', 'postSortLists.media', 'postSortLists', 'media',
        ...
        $data['previousPost'] = Post::find($previous);

        $data['nextPost'] = Post::find($next);

        $data['comments'] = Comment::with('users')->wherePostId($post->id)->where('status', 1)->latest()->get();
        $data['totalComments'] = $data['comments']->count();
        $data['relatedPosts'] = Post::with('category')->where('id', '!=', $post->id)->where('category_id',
            $post->category_id)->where('post_types', $post->post_types)->whereVisibility(Post::VISIBILITY_ACTIVE)->get();

        if (!empty($post->post_types)) {

            if ($post->post_types == Post::ARTICLE_TYPE_ACTIVE) {
                ...
                return redirect(route('front_new.home'));
            }
        } else {
            return redirect(route('front_new.home'));
        }
